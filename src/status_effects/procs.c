...
void stat_boost(u8 bank, u8 stat_id, s8 amount, u8 inflicting_bank)
{
    if (!amount)
        return;
    // back up cbs
    ...
    for(u8 i = 0; i < BANK_MAX; i++) {
        ...
        if ((abilities[ability].on_stat_boost_mod) && (ACTIVE_BANK(i))) {
            u8 cb_id = add_callback(CB_ON_STAT_BOOST_MOD, 0, 0, i, (u32)abilities[ability].on_stat_boost_mod);
            CB_MASTER[cb_id].data_ptr = STORE_ADDITIONAL_DATA_FOR_STAT_BOOST_MOD(inflicting_bank, (amount < 0), ABS(amount));
        }
    }
    ...
    for(u8 i = 0; i < BANK_MAX; i++) {
        ...
        if ((abilities[ability].after_stat_boost_mod) && (ACTIVE_BANK(i))) {
            u8 cb_id = add_callback(CB_AFTER_STAT_BOOST_MOD, 0, 0, i, (u32)abilities[ability].after_stat_boost_mod);
            CB_MASTER[cb_id].data_ptr = STORE_ADDITIONAL_DATA_FOR_STAT_BOOST_MOD(inflicting_bank, (new_amount < 0), delta);
        }
    }
