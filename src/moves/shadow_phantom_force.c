...
/* Shadow force */
u8 shadow_force_on_before_move(u8 user, u8 src, u16 move, struct anonymous_callback* acb)
{
    if (user != src) return true;
    // shadow force two-turn effect
    return shadow_force_phantom_force_effect(user, src, STRING_CHARGE_SHADOW_FORCE);
}
