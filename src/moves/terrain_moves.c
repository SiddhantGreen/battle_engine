...
u8 electric_terrain_on_effect(u8 user, u8 src, u16 move, struct anonymous_callback* acb)
{
    ...
    battle_master->field_state.is_electric_terrain = true;
    enqueue_message(NULL, NULL, STRING_ELECTRIC_TERRAIN_START, NULL);
    return true;
}
...
u8 grassy_terrain_on_effect(u8 user, u8 src, u16 move, struct anonymous_callback* acb)
{
    ...
    battle_master->field_state.is_grassy_terrain = true;
    enqueue_message(NULL, NULL, STRING_GRASSY_TERRAIN_START, NULL);
    return true;
}
...
u8 misty_terrain_on_effect(u8 user, u8 src, u16 move, struct anonymous_callback* acb)
{
    ...
    CB_MASTER[id].delay_before_effect = 5;
    battle_master->field_state.is_misty_terrain = true;
    enqueue_message(NULL, NULL, STRING_MISTY_TERRAIN_START, NULL);
    return true;
}
...
u8 psychic_terrain_on_effect(u8 user, u8 src, u16 move, struct anonymous_callback* acb)
{
    ...
    CB_MASTER[id].delay_before_effect = 5;
    battle_master->field_state.is_psychic_terrain = true;
    enqueue_message(NULL, NULL, STRING_PSYCHIC_TERRAIN_START, NULL);
    return true;
}
