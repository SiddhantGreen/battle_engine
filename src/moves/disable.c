...
u8 disable_on_effect_cb(u8 user, u8 src, u16 move, struct anonymous_callback* acb)
{
	...
	u16 last_move = p_bank[defender]->b_data.last_move;
	...
	if (p_bank[defender]->b_data.disable_used_on_slot < 4)
		return false;

	// add the move to the disabled moves array in pbank
	if (p_bank[defender]->b_data.disabled_moves[index] == 0) {
		p_bank[defender]->b_data.disabled_moves[index] = 4; // disable move index for 4 turns
		p_bank[defender]->b_data.disable_used_on_slot = index; // disable move index for 4 turns
